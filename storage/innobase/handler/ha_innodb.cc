...
#ifndef WITH_INNODB_LEGACY_FOREIGN_STORAGE
			/* MDEV-21652: transaction might be started by
			check_legacy_fk() */
	...
	my_bool		discard)	/*!< in: TRUE if discard, else import */
{
	...
	if (!m_prebuilt->table) {
		...
		err = dict_load_foreigns(m_user_thd, m_prebuilt->table, NULL,
					 NULL, false, DICT_ERR_IGNORE_FK_NOKEY);
		if (err != DB_SUCCESS) {
			dict_table_close(m_prebuilt->table, true, false);
	...
	void* user_arg) /*!< in: fk_legacy_data */
{
	...
	if (id.str == NULL) {
		...
		if (prefix_len >= fld->len) {
			ib_foreign_warn(d.trx, DB_CANNOT_ADD_CONSTRAINT,
					d.s->table_name.str,
	...
	table.length = filename_to_tablename(table.str, dst_table,
					     sizeof(dst_table));
	ut_a(table.length);
	table.str = dst_table;
	...
	if (db.length) {
		db.length
			= filename_to_tablename(db.str, dst_db, sizeof(dst_db));
		ut_a(db.length);
		db.str = dst_db;
	}
			  ...
			  TABLE_SHARE* share)
{
	...
	pars_info_bind_function(info, "fk_upgrade_create_fk",
				fk_upgrade_create_fk, &d);
	pars_info_bind_function(info, "fk_upgrade_add_col", fk_upgrade_add_col,
				&d);
	pars_info_bind_function(info, "fk_upgrade_push_fk", fk_upgrade_push_fk,
				&d);
	...
	if (share->referenced_keys.elements > table->referenced_set.size()) {
		...
		for (const Table_name& t : tables_missing) {
			...
			dict_table_t* for_table
				= dict_table_check_if_in_cache_low(buf);
			if (!for_table) {
